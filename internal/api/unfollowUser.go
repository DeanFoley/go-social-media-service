...
func UnfollowUser(w http.ResponseWriter, r *http.Request) {
	...
	go db.UnfollowUser(request.Follower, request.Target, resultChan, errorChan)

	select {
	case <-resultChan:
		w.WriteHeader(http.StatusOK)
		w.Write([]byte(fmt.Sprintf("User %s is no longer following %s!", request.Follower, request.Target)))
	case err := <-errorChan:
